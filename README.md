...
* **유사한 동작 방식의 예시 (Iterator)**: 자바의 컬렉션 프레임워크에서 사용하는 **`Iterator.hasNext()`** 및 **`Iterator.next()`** 조합과 매우 유사하게 동작합니다.   

### 6. JavaDocs를 이용한 API 문서화
**JavaDocs** 는 코드의 의도와 사용법을 소스 코드 자체에 기입하는 것을 의미합니다. 이렇게 기입한 정보는 IDE에 의해 호출하는 곳에서 볼 수 있게 됩니다.
* **작성법**: `/** ... */` 형식을 사용하며, `@param`(매개변수), `@return`(반환값), `@throws`(예외) 등의 태그를 통해 구조화된 정보를 제공합니다.
   
* **장점**:
* **협업 효율성**: 다른 개발자가 복잡한 로직을 직접 읽지 않아도 메서드의 역할과 제약 사항을 즉시 파악할 수 있습니다.
* **IDE 연동**: IntelliJ와 같은 툴에서 메서드 호출 시 해당 설명을 팝업으로 띄워주어 개발 속도를 높여줍니다.
   
* **단점**:
* **유지보수 비용**: 로직이 변경되었을 때 주석을 함께 수정하지 않으면 "거짓 정보"가 담긴 문서가 될 위험이 있습니다.
